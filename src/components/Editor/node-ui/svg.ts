...
export class Svg {
    ...
    constructor(dest: HTMLElement = document.body) {
        ...
        let {width, height} = this.dest.getBoundingClientRect();
    ...
    private updateSize() {
        let {width, height} = this.dest.getBoundingClientRect();
        if (this.cashSize === '' + width + height) return;

        this.cashSize = '' + width + height;
        this.setProperty(this.svg, {width: width, height: height})
        this.setView(0, 0, width, height);
        this.width = width;
        this.height = height;
    ...
    public getView() {
        let strAttr = this.svg.getAttribute('viewBox');
        if (!strAttr) return {x: 0, y: 0, width: 0, height: 0}
        const [x, y, width, height] = strAttr!.split(' ').map(it => +it);
    ...
    private handlerMouseMove(e: MouseEvent) {
        ...
        // была проблема с объектами которые встраивались через foreignObject
